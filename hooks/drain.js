...
export async function drain() {
  ...
  const context = [
    ...
    ...lines,
    'Rules:',
    '1. FIRST, show the user every message VERBATIM — sender name and full text in a quote block,',
    '   word for word. The UI does not render these messages; your reply is the only place the user',
    '   can read them. Never paraphrase in place of quoting.',
    '2. Reply IN YOUR VISIBLE RESPONSE by addressing the sender on its own line:',
    '       @alice: good catch — incorporated.',
    '   The Hub relays such lines automatically and marks that sender\'s messages acted. Your chat',
    '   response IS the reply — do NOT hold a separate conversation via Bash sends.',
    '3. Disposition EVERY message listed above this way — none may be silently ignored.',
    `4. If the user writes "@<name>: text", include that line verbatim in your response — it relays to`,
    '   <name> automatically.',
    'Fallback CLI (only for messaging someone without replying, or explicit incorporate/decline detail):',
    `  node ${REPO}/bin/hub.js send --as ${HUB_USER} --to <name> "text"`,
